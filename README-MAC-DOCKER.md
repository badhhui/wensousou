...
docker run --rm --platform linux/arm64 \
  ...
  bash -lc 'cd /workspace/dist/offline-kit && ./check-target.sh wensousou_1.1.5_arm64.deb'
```
...
Target preflight passed. Install with:
  sudo dpkg -i wensousou_1.1.5_arm64.deb
```
...
```bash
cd offline-kit
./check-target.sh wensousou_1.1.5_arm64.deb
sudo dpkg -i wensousou_1.1.5_arm64.deb
